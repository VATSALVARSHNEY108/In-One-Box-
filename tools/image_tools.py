...
def display_tools():
    ...
    # Add special cat animation for Image Tools section
    st.markdown("""
    <style>
        .image-tools-cat-container {
            position: fixed;
            top: 0;
            left: 0;
            width: 100vw;
        ...
        .special-trail {
            ...
            border-radius: 50%;
            pointer-events: none;
            animation: specialTrailFade 2s ease-out forwards;
        }

        @keyframes specialTrailFade {
            from { opacity: 1; transform: scale(1) rotate(0deg); }
            to { opacity: 0; transform: scale(0) rotate(720deg); }
        }
    </style>

    <div class="image-tools-cat-container" id="imageToolsCatContainer">
        <img class="image-tools-cat" id="imageToolsCat" src="https://i.ibb.co/8N1n8cL/cat.png" alt="Cat for Image Tools">
    </div>

    <script>
        // Enhanced cat animation for Image Tools - reinitialize on each render
        (function() {
            // Check if we need to initialize or reinitialize
            const cat = document.getElementById("imageToolsCat");
            const container = document.getElementById("imageToolsCatContainer");
            ...
            // Clean up any existing mouse listeners for Image Tools cat
            if (window.imageToolsMouseHandler) {
                document.removeEventListener("mousemove", window.imageToolsMouseHandler);
            }
            ...
            // Enhanced cat following animation
            function updateImageToolsCatPosition() {
                const currentCat = document.getElementById("imageToolsCat");
                if (!currentCat) {
                    // Cat element is gone, stop animation
                    return;
                }
                ...
                // Faster, more responsive movement for Image Tools
                catX += dx * 0.12;
                catY += dy * 0.12;

                // Create transform with flip and slight bounce effect
                let transform = `translate(${catX - 45}px, ${catY - 45}px)`;

                // Enhanced flip animation
                if (dx > 2 && isFlipped) {
                    isFlipped = false;
                    transform += ' scaleX(1)';
                } else if (dx < -2 && !isFlipped) {
                    isFlipped = true;
                    transform += ' scaleX(-1)';
                } else if (isFlipped) {
                    transform += ' scaleX(-1)';
                ...
                currentCat.style.transform = transform;
                animationFrame = requestAnimationFrame(updateImageToolsCatPosition);
            }

            // Create special trailing effect
            function createSpecialTrail(x, y) {
                const currentContainer = document.getElementById("imageToolsCatContainer");
                ...
                currentContainer.appendChild(trail);
            ...
            function createSparkle(x, y) {
                ...
                sparkle.style.left = (x - 4 + Math.random() * 20 - 10) + 'px';
                sparkle.style.top = (y - 4 + Math.random() * 20 - 10) + 'px';
                currentContainer.appendChild(sparkle);
            ...
            // Enhanced mouse tracking with special effects
            window.imageToolsMouseHandler = function(e) {
                mouseX = e.clientX;
                mouseY = e.clientY;

                // Create enhanced trail
                if (Math.random() < 0.4) {
                    createSpecialTrail(e.clientX, e.clientY);
                }

                // Create sparkles occasionally
                if (Math.random() < 0.1) {
                    createSparkle(catX, catY);
                }
            };

            document.addEventListener("mousemove", window.imageToolsMouseHandler);

            // Initialize cat position and start animation
            cat.style.transform = `translate(${catX - 45}px, ${catY - 45}px)`;
            updateImageToolsCatPosition();
        })();
    </script>
    ...
    tool_categories = {
        ...
        "Miscellaneous": [
            "Icon Generator", "Placeholder Creator", "Image Statistics", "Format Info", "EXIF Viewer", "Cat Animation"
        ]
    }
    ...
    if selected_tool == "Format Converter":
        ...
        exif_viewer()
    elif selected_tool == "Cat Animation":
        cat_animation()
    else:
...
def cat_animation():
    """Interactive cat follows mouse animation"""
    create_tool_header("Cat Animation", "Watch a cute cat follow your mouse cursor around the screen!", "🐱")

    st.markdown("""
    ### 🐱 Interactive Cat Animation

    Enjoy this fun interactive animation where a cute cat follows your mouse cursor around the screen!
    This is perfect for:
    - **Taking a break** from serious work
    - **Testing mouse responsiveness** 
    - **Adding some fun** to your day
    - **Demonstrating interactive animations**

    **Instructions:** Simply move your mouse around the screen and watch the cat follow!
    """)

    # Cat animation HTML with enhanced features
    cat_html = """
    <style>
        .cat-container {
            position: fixed;
    ...
    <div class="cat-container">
        <div class="instructions">
            🐱 <strong>Cat Follows Mouse!</strong><br>
            Move your mouse around to see the cat follow you.<br>
            <small>The cat leaves a trail and paw prints!</small>
        </div>
        <img class="cat" id="cat" src="https://i.ibb.co/8N1n8cL/cat.png" alt="Cat">
    </div>
    ...
    <script>
        ...
        function animateCat() {
            ...
            catX += dx * 0.1; // Smooth following speed
            catY += dy * 0.1;

            if (cat) {
                cat.style.transform = `translate(${catX - 40}px, ${catY - 40}px)`;
        ...
        document.addEventListener("mousemove", (e) => {
            ...
            // Occasionally create paw prints
            if (Math.random() < 0.1) {
        ...
        function createTrail(x, y) {
            ...
            document.querySelector('.cat-container').appendChild(trail);
        ...
        function createPawPrint(x, y) {
            ...
            document.querySelector('.cat-container').appendChild(paw);
    ...
    # Display the cat animation
    components.html(cat_html, height=600, scrolling=False)
    ...
    with col1:
        st.markdown("""
        ### 🎮 Features:
        - **Smart Following**: Cat smoothly follows your cursor
        - **Trail Effects**: Beautiful particle trail behind your mouse
        - **Paw Prints**: Cat leaves cute paw prints as it moves
        - **Direction Aware**: Cat faces the direction it's moving
    ...
    with col2:
        st.markdown("""
        ### 💡 Fun Facts:
        - The cat image is hosted on a reliable CDN
        - Uses smooth CSS transitions for fluid movement
        - Particle effects are created with JavaScript
        - Trail effects automatically fade out
    ...
    with col_a:
        if st.button("🔄 Reload Animation", type="secondary"):
            st.rerun()

    with col_b:
        st.markdown("[🐱 Get Cat Images](https://www.pexels.com/search/cat/)")

    with col_c:
        st.markdown("[🎨 Learn Web Animation](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Animations)")
