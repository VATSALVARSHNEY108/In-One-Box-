...
def add_to_favorites(tool_name: str, category: str, module: str = ""):
...
def remove_from_favorites(tool_name: str, category: str):
    ...
    if len(st.session_state.favorites) < original_length:
        show_success_message(f"Removed {tool_name} from favorites!")
        return True
    return False
...
def display_favorites_section():
    """Display favorites section in the dashboard"""
            ...
            with cols[j]:
                with st.container():
                    ...
                    <div style="
                        ...
                        text-align: center;
                    ">
                        <h4 style="margin: 0; color: #4A90E2;">⭐ {fav['name']}</h4>
                        <p style="margin: 0.5rem 0; opacity: 0.7; font-size: 0.8rem;">
                            📁 {fav['category']}
                        </p>
                        <p style="margin: 0; opacity: 0.6; font-size: 0.7rem;">
                            Used {fav.get('usage_count', 0)} times
                        </p>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    with col1:
                        if st.button(f"Open", key=f"open_fav_{fav['id']}", use_container_width=True):
                            update_favorite_usage(fav['name'], fav['category'])
                            navigate_to_tool(fav['category'], fav['name'])
                    with col2:
                        if st.button("🗑️", key=f"remove_fav_{fav['id']}", help="Remove from favorites"):
    ...
    for i, tool_name in enumerate(st.session_state.recent_tools[-5:]):  # Show last 5 tools
        with cols[i]:
            ...
            <div style="
                ...
                margin: 0.2rem 0;
            ">
                <p style="margin: 0; font-size: 0.9rem; font-weight: 500;">{tool_name}</p>
            </div>
            """, unsafe_allow_html=True)
